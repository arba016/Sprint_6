...
from locators.order_page_locators import OrderPageLocators, OrderPageAboutRentLocators, ApproveOrderPageLocators, StatusOrderWindow
...
class OrderPage(BasePage):
    ...
    def input_adress(self, adress):
        self.send_text(OrderPageLocators.ADRESS_INPUT, adress)
    ...
    def date_rent(self):
        ...
        with allure.step(f"Выбираем дату аренды {tomorrow_date}"):
            self.click_element(OrderPageAboutRentLocators.DATE_INPUT)
            self.send_text(OrderPageAboutRentLocators.DATE_INPUT, tomorrow_date)
            self.click_by_esc(OrderPageAboutRentLocators.DATE_INPUT)
    ...
    def click_rental_perriod_button(self):
        self.click_element(OrderPageAboutRentLocators.RENTAL_PERIOD_BUTTON)

    @allure.step("Выбираем период аренды 'три дня'")
    def rent_period(self):
        self.click_element(OrderPageAboutRentLocators.THREE_DAYS)

    @allure.step("Выбираем цвет самоката: {color}")
    def choose_scooter_color(self, color):
        colors = {
            "grey": OrderPageAboutRentLocators.GRAY_COLOR,
            "black": OrderPageAboutRentLocators.BLACK_COLOR,
        }
    ...
    def comment_for_courier(self, comment):
        self.send_text(OrderPageAboutRentLocators.COMMENT, comment)

    @allure.step("Кликнуть на кнопку 'Заказать' в окне 'Про аренду' ")
    def click_order_button(self):
        self.click_element(OrderPageAboutRentLocators.ORDER_BUTTON)

    @allure.step("Кликнуть на кнопку 'Да' в окне подтверждения заказа")
    def click_yes_button(self):
        self.click_element(ApproveOrderPageLocators.YES_BUTTON)

    @allure.step("Проверяем, что открылось окно с информацией о заказе")
    def check_window_complete_order(self):
        self.wait_visibility(StatusOrderWindow.STATUS_ORDER_TEXT)

    @allure.step("Получаем текст окна успешного заказа")
    def get_complete_order_text(self):
        return self.wait_visibility(StatusOrderWindow.STATUS_ORDER_TEXT).text

    @allure.description("Заполняем форму заказа самоката")
    def order_scooter(self, name, surname, adress, tel_number, comment, color):
